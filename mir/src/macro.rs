...
macro_rules! import {
    ($path:expr) => {
        ::ln_mir::Import::package($path)
    };
    ($path:expr, $($imports:ident),*) => {
        ::ln_mir::Import::new($path, vec![$(stringify!($imports)),*])
    };
    (pub $path:expr, $($imports:ident),*) => {
        ::ln_mir::Import::new($path, vec![$(stringify!($imports)),*]).public()
    };
}
...
macro_rules! arg {
    ($name:ident : $ty:expr) => {
        ::ln_mir::FnArg {
            name: ::ln_mir::ArgIdent::from(stringify!($name)),
            ty: $ty.to_string(),
            default: None,
            treatment: None,
        }
    ...
    ($name:ident : $ty:expr , $default:expr) => {
        ::ln_mir::FnArg {
            name: ::ln_mir::ArgIdent::from(stringify!($name).to_string()),
            ty: $ty.to_string(),
...
macro_rules! field {
    (pub(crate) $name:ident : $ty:expr) => {
        ::ln_mir::Field {
            name: ::ln_mir::Name::new(stringify!($name)),
            ty: ($ty).into(),
            visibility: ::ln_mir::Visibility::Crate,
            ..Field::default()
        }
    };
    (pub $name:ident : $ty:expr) => {
        ::ln_mir::Field {
            name: ::ln_mir::Name::new(stringify!($name)),
            ty: ($ty).into(),
            visibility: ::ln_mir::Visibility::Public,
            ..Field::default()
        }
    };
    ($name:ident : $ty:expr) => {
...
macro_rules! lit {
    ($($arg:tt)*) => {
        ::ln_mir::Literal(format!($($arg)*), false);
    };
}