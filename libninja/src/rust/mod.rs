...
fn write_model_module(spec: &HirSpec, opts: &Config) -> Result<()> {
...
fn shared_oauth2_flow(auth: &Oauth2Auth, spec: &HirSpec, opts: &Config) -> TokenStream {
...
fn write_lib_rs(spec: &HirSpec, extras: &Extras, opts: &Config) -> Result<()> {
    let src_path = opts.dest.join("src");
...
fn write_request_module(spec: &HirSpec, opts: &Config) -> Result<()> {
    let src_path = opts.dest.join("src");
...
fn write_examples(spec: &HirSpec, opts: &Config) -> Result<()> {
