...
use openapiv3::{
    ArrayType, OpenAPI, Operation, Parameter, PathItem, ReferenceOr, RefOr, Schema, SchemaKind,
    Type,
};
use tracing_ez::span;
...
use crate::extractor;
use crate::extractor::{
    is_primitive, schema_ref_to_ty, schema_ref_to_ty_already_resolved, schema_to_ty,
};
...
pub fn extract_param(param: &ReferenceOr<Parameter>, spec: &OpenAPI) -> Result<hir::Parameter> {
    ...
    let ty = schema_ref_to_ty_already_resolved(param_schema_ref, spec, schema);
    Ok(hir::Parameter {
        doc: None,
